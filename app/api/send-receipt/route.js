...
export async function POST(req) {
  try {
    ...
    const templateName =
      receiptData.organization.name === "Seeshan"
        ? receiptData.noCustomMessage
          ? "seeshan-receipt-no-text.hbs"
          : "seeshan-receipt.hbs"
        : receiptData.noCustomMessage
        ? "mim-receipt-no-text.hbs"
        : "mim-receipt.hbs";
    ...
    const browserlessPDFResponse = await fetch(
      `https://chrome.browserless.io/pdf?token=${process.env.BROWSERLESS_TOKEN}`,
      {
        ...
        body: JSON.stringify({
          ...
          options: {
            ...
            format: `${receiptData.noCustomMessage ? "A5" : "A4"}`,
          },
        }),
      }
    );
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("Donor").getPublicUrl(filePath);
